################################################################################
#this is Catarina's test version of the code
# imputeEffort.R
...
imp_model = 1      # If run_all=FALSE, which model to
  ...
  # Need to make sure that lakes in lake_dat are in alphabetical order
  ...
  effort_dat_lake <- cbind(effort_dat,lake_dat_obs) %>%
                as.data.frame() %>% 
  ...
  # Ground observations
  effort_ground <- effort_dat_lake %>%
                   filter( method=="GR" ) %>%
                   rename( ground_count = angler_count ) %>%
                   select( -method )

  # Camera observations
  effort_cam <- effort_dat_lake %>%
                filter( method=="CAM" ) %>%
  ...
  effort <- effort_dat_lake[0, ]

  # Get unique locations from effort dataset
  locations <- effort_dat_lake %>%
...
summary(effort)
  ...
  # Zero camera observations and zero ground count - true zero
  zero_cam <- effort %>%
              group_by( p_seen, area, weekend, time_of_day, hour, year,
                        lake_camera, lake_ID,lakeyear ) %>% #,julian_day
...
dim(pos_cam)
  ...
  summary(effort_cam)
  sort(unique(effort_cam$lakeyear))
  ...
  #summary(zero_cam)# Lake_ID present
  ...
  for( i in lakeyears ) {
    n_days_i <- length(unique(imputation_dat$julian_day[imputation_dat$lakeyear == i]))
    ...
    if( days_prop < cutoff ) {
      ...
      #added this to address the NAs
      false_zero <- filter( false_zero, lakeyear!=i )
      pos_cam <- filter( pos_cam, lakeyear!=i )
      zero_cam <- filter( zero_cam, lakeyear!=i )
  ...
  #update lakeyears to only the data that passes the cutoff
  ...
  sort(unique(imputation_dat$lakeyear))
  sort(unique(false_zero$lakeyear) ) # has 201119, 201126,201131 missing from imputation_dat$lakeyear
  sort(unique(pos_cam$lakeyear) ) # 201119,201126,201131,201135 
  sort(unique(zero_cam$lakeyear) )
  ...
  zero_cam$lakeyear       <- match(zero_cam$lakeyear,lakeyears)
 ...
 #PROBLEM
  ...
  # this filter should be across lakes, not lakeyear
    ...
    zero_cam        <- filter(zero_cam,       lakeyear==one_ly)
  ...
  summary(imputation_zero)
  as.data.frame(imputation_zero)
  ...
  #for( ly in 1:n_lakeyears ) {
    ...
    n_days_per_lakeyear <- length(obs_days)

    # Days available for imputation
    day_seq              <- 1:n_days_per_lakeyear
    all_days[,day_seq] <- obs_days
    ...
    if( length( zero_days_temp ) > 0 ) {
      zero_lakeyears[,zero_days_temp] <- one_ly
      zero_days[,zero_days_temp]      <- zero_days_temp
      zero_nobs[,zero_days_temp]      <- imputation_zero_ly %>%
                                           group_by(julian_day) %>% 
                                           summarise(nobs=length(julian_day)) %>%
                                           .$nobs
    }
    if( length( pos_days_temp ) > 0 ) {
      pos_lakeyears[,pos_days_temp] <- one_ly
      pos_days[,pos_days_temp]      <- pos_days_temp
      pos_nobs[,pos_days_temp]      <- imputation_pos_ly %>%
                                         group_by(julian_day) %>% 
                                         summarise(nobs=length(julian_day)) %>%
                                         .$nobs
    }
...
#added for single lake
  pseen_lakecam <- effort_cam %>%
           group_by(lake_camera,p_seen) %>% 
           summarise() %>%
           .$lake_camera
  
pseen_ol<-pseen[ na.exclude(match(lakecams,pseen_lakecam))]
pseen_lac<- pseen_lakecam[ na.exclude(match(lakecams,pseen_lakecam))]
 ...
 names(imputation_dat )
  ...
  lakeyear_data <- imputation_dat %>%
                   ...
                   select( lakeyear, year, area, region, access,
