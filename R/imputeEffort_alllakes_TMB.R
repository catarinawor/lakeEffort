...
#                  imp_model = 1      # If run_all=FALSE, which model to
                                     # use for imputation?
  ...
  effort_dato$lake_ID <- match( effort_dato$name, lake_names )
  ...
  for( ly in 1:n_lakeyears ) {
    ...
    aggregate(imputation_zero_ly$julian_day,list(imputation_zero_ly$julian_day),length)
  ...
  agg_pos_df<-aggregate(imputation_pos$cam_count,
               list(lakeyear=imputation_pos$lakeyear,
                julian_day=imputation_pos$julian_day),length)
  
  aggond<-paste0(agg_pos_df$lakeyear,agg_pos_df$julian_day)
  ond<-paste0(imputation_pos$lakeyear,imputation_pos$julian_day)

  pos_agg_ind<-(match(ond,aggond))
  

  agg_zero_df<-aggregate(imputation_zero$cam_count,
               list(lakeyear=imputation_zero$lakeyear,
                julian_day=imputation_zero$julian_day),length)
  

  aggznd<-paste0(agg_zero_df$lakeyear,agg_zero_df$julian_day)
  znd<-paste0(imputation_zero$lakeyear,imputation_zero$julian_day)

  zero_agg_ind<-(match(znd,aggznd))
  
  
  allEff<-merge(x = agg_pos_df, y = agg_zero_df,
             by = c("lakeyear","julian_day"), all = TRUE)
  allEffind <- paste0(allEff$lakeyear,allEff$julian_day)
  

  allEff_ind_zero<-match(allEffind, aggznd)
  allEff_ind_pos<-match(allEffind, aggond)

  length(allEffind)
  length(allEff_ind_zero)
  length(aggznd)

  max(allEff_ind_zero,na.rm=T)
  max(aggznd,na.rm=T)


  sort(unique(allEff_ind_zero)-1)

  which(is.na(allEff_ind_zero))
  which(is.na(allEff_ind_pos))
  #745 1435

  TMB_data<- list(n_lyrs   = n_lakeyears,
                  #nlc      = n_lakecams,
                  nzz      = nrow(zero_cam),
                  nzp      = nrow(false_zero),
                  npp      = nrow(pos_cam),
...
#integer vectors
                  ...
                  zpcount  = false_zero$ground_count,
                  ...
                  zero_agg_ind = match(znd,aggznd)-1,
                  lzero_agg_ind = max(match(znd,aggznd)),
                  pos_agg_ind = match(ond,aggond)-1,
                  lpos_agg_ind = max(match(ond,aggond)),
                  allEff_ind_zero = match(allEffind, aggznd)-1,
                  allEff_ind_pos = match(allEffind, aggond)-1,
                  nallEff_ind = length(allEff_ind_pos)

#
                    )
length(allEff_ind_zero)
#======================================================
#Adjust index for TMB

TMB_data$lc_zz<-match(TMB_data$lc_zz,1:51)-1
TMB_data$lc_zp<-match(TMB_data$lc_zp,1:51)-1
TMB_data$lc_pp<-match(TMB_data$lc_pp,1:51)-1
TMB_data$lc_iz<-match(TMB_data$lc_iz,1:51)-1
TMB_data$lc_ip<-match(TMB_data$lc_ip,1:51)-1

TMB_data$ly_iz<-TMB_data$ly_iz-1
TMB_data$date_iz<-TMB_data$date_iz-121
TMB_data$i_iz<-TMB_data$i_iz-1



sort(unique(TMB_data$lc_pp))
sort(unique(TMB_data$lc_pp))
sort(unique(TMB_data$lc_pp))
sort(unique(TMB_data$lc_iz))
sort(unique(TMB_data$lc_ip))

TMB_data$nip



max(imputation_zero_lakecams)
max(imputation_pos_lakecams)
...
#write.csv(imputation_zero,"imputation_zero.csv")
#write.csv(imputation_pos,"imputation_pos.csv")
  ...
  lakeyear_data <- imputation_dat %>%
                   distinct(lake_ID, lakeyear, year, area, region, access, launch,
