...
{  
  ...
  effort_dat$lake_ID <- match( effort_dato$name, lake_names )
...
save(jags_data,file="jags_data.Rdata")
  ...
  if( run_all ) {
    ...
    model_file <- list( model_1 = "/Users/catarinawor/Documents/lake_effort/JAGS/input/ImputeEff_h_1.txt")#,  # no covariates         
    ...
    for( i in 1:n_models ) {
     ...
     jags_output$BUGSoutput$mean$L[sort(unique(jags_data$lc_zp
      ))]
      jags_output$BUGSoutput$mean$L
